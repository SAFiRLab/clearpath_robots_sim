...
public:
    ...
    : DynamicModel(3, 2, 3), constraints_(constraints), track_width_(track_width)
    {}
    ...
    casadi::MX f(const casadi::MX &x, const casadi::MX &u, const casadi::MX &dt) override
    {
        ...
        x_next(0) = x(0) + dt * u(0) * cos(x(2));
        x_next(1) = x(1) + dt * u(0) * sin(x(2));
        x_next(2) = x(2) + dt * u(1);
        //x_next(3) = x(3) + dt * u(0);
        //x_next(4) = x(4) + dt * u(1);
    ...
    void getControlBounds(std::vector<double> &lb,  std::vector<double> &ub) const override
    {
        ...
        if (constraints_.min_max_velocity_set)
        {
            lb[0] = constraints_.min_velocity;
            ub[0] = constraints_.max_velocity;
        }

        if (constraints_.min_max_yaw_rate_set)
        {
            lb[1] = constraints_.min_yaw_rate;
            ub[1] = constraints_.max_raw_rate;
        }
